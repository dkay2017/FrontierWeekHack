...
| S-3 | **Challenge 4 — Foundry portal workflow** | S | Runbook `docs/runbooks/challenge-4-portal-workflow.md` — build `evidence-gap → precedent-strategist → critic → End` in the portal designer. Agents-as-assets half done via the provisioner. **User runs it in the portal.** |
