...
| ~~S-6~~ | **New architecture SVG + deep TDD pass** — ✅ **done 2026-09-09.** `Care-Approval-IQ-Architecture_Design-V4.svg` (adds the dashed "UPSTREAM — NOT BUILT" band, eval 24 + baseline; the current SVG was already MAF-aware). `Care-Approval-IQ-TDD-V4.md` — deep rewrite: MAF Workflow orchestration, 5 agents (incl. `claims-extraction`), §3.1 + §7.3 with the measured baseline numbers, §12 TD-1/TD-2/TD-3 for MAF + new TD-1a, `ReadyToSubmit`. `build-tdd-docx.py` bumped to V4 (DEST `Care-Approval-IQ-TDD-V4.docx`, "Version 4") — **needs `pip install python-docx` + one run** to produce the .docx (no Python on the authoring machine). ARCHITECTURE.md light-touched (5 agents, TD-1, eval 24). Old TDD.md / .docx / SVG kept untouched. | — | |
| S-7 | **Pitch + 5-point doc** | M | Submission artifact. Draw from TDD-V4 §11 + §11.1. |
